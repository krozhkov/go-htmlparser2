...
func parseDOM(t *testing.T, str string) []*dom.Node {
	n, err := dom.ParseDOM(str, &parser.ParserOptions{LowerCaseAttributeNames: true, DecodeEntities: true, RecognizeSelfClosing: true})
	require.Nil(t, err)
	return n
}
...
func TestTraversal(t *testing.T) {
	...
	t.Run("hasAttrib", func(t *testing.T) {
		t.Run("doesn't throw on text nodes", func(t *testing.T) {
			assert.False(t, HasAttrib(parseDOM(t, "textnode")[0], "some-attrib"))
		})

		t.Run("returns `false` for Object prototype properties", func(t *testing.T) {
			assert.False(t, HasAttrib(parseDOM(t, "<div><h1></h1>test<p></p></div>")[0], "constructor"))
		})

		t.Run("should return `false` for \"null\" values", func(t *testing.T) {
			div := parseDOM(t, "<div class=test>")[0]
	...
	t.Run("nextElementSibling", func(t *testing.T) {
		t.Run("return Element if found", func(t *testing.T) {
			dom := parseDOM(t, "<div><h1></h1>test<p></p></div>")[0]
		...
		t.Run("return null if not found", func(t *testing.T) {
			dom := parseDOM(t, "<div><p></p>test</div>")[0]
			firstNode := dom.FirstChild()
		...
		t.Run("does not ignore script tags", func(t *testing.T) {
			dom := parseDOM(t, "<div><p></p><script></script><p></div>")[0]
			firstNode := dom.FirstChild()
	...
	t.Run("prevElementSibling", func(t *testing.T) {
		...
		t.Run("return null if not found", func(t *testing.T) {
			dom := parseDOM(t, "<div>test<p></p></div>")[0]
			lastNode := dom.Children[1]

			assert.Nil(t, PrevElementSibling(lastNode))
		})
		t.Run("does not ignore script tags", func(t *testing.T) {
			dom := parseDOM(t, "<div><p></p><script></script><p></p></div>")[0]
			lastNode := dom.Children[2]
